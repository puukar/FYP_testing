...
class ResumeRepository:
    ...
    def save_skill_gap_analysis(
        ...
        """
        Save skill gap analysis results (OLD pipeline - kept as-is so the
        currently-live app doesn't break).
        """
        try:
            analysis_data = {
                "user_id": user_id,
    ...
    def save_full_analysis(
        ...
        overall_score: float,          # expects a 0.0-1.0 fraction from scorer.py
        recommendations: List[str],
    ) -> Optional[Dict[str, Any]]:
        """
        Stage 17 — Save results from the NEW analysis pipeline
        (raw_text_extractor -> resume_structurer -> ideal_profile_generator ->
        preprocessor -> tfidf -> similarity -> scorer -> skill_gap -> recommender).

        Converts overall_score and section_scores from 0.0-1.0 fractions
        (as returned by scorer.py / similarity.py) into 0-100 numeric values
        to match the skill_gaps table's numeric(5,2) match_score column and
        keep section_scores consistent in the same 0-100 scale.

        Kept separate from save_skill_gap_analysis() (the old pipeline's
        save method) so the currently-live app doesn't break while this
        new pipeline is being built.
        """
        try:
            section_scores_pct = {
            ...
            analysis_data = {
                ...
                "matched_skills": json.dumps(matched_skills),
                "missing_skills": json.dumps(missing_skills),
                "extra_skills": json.dumps(extra_skills),
                "match_score": round(overall_score * 100, 2),
                "section_scores": json.dumps(section_scores_pct),
            ...
            print(f"Error saving full analysis: {e}")
            return None
        ...
        try:
            ...
            if response.data:
                for analysis in response.data:
                    ...
                    if isinstance(analysis.get("extra_skills"), str):
                        analysis["extra_skills"] = json.loads(analysis["extra_skills"])
                    if isinstance(analysis.get("section_scores"), str):
                        analysis["section_scores"] = json.loads(analysis["section_scores"])
